...
1, the required switch opens: required = true / &amp; checkList contains value (current form key + ordinal value), which does not satisfy the situation.
2, required switch open: required = true &amp; &amp; checkList contains values (current form key value + serial number value) to satisfy the situation, but the form content does not meet the verification requirements
3, the form of the overall submission triggering the above 2 points do not meet the requirements, the form submitted changes checkList contains'all-check', can open the first level verification switch.
4. When defocus / change is verified, change checkList = conform to the current form key value + ordinal value, and open the first level verification switch.
5, when emptying the reset form, set checkList = [] only (the incoming value is'no-check').
  ...
  mutations: {
    SETCUSTOMCHECK: (state, data) => {
      if (data === 'no-check') {
        ...
        if (!ns.base.arrContainObj(state.check.checkList, data)) {
    ...
    AUTOFORMCOUNTER: (state, data) => {
      ...
      switch (type) {
        ...
        default:
          ns.error.throw(
            "the auto-from counter methods is error, only 'register', 'add', 'reduce'，and 'empty' methods，find it."
          );
      }
    },
