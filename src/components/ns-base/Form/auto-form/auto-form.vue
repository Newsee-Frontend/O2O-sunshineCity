...
export default {
  ...
  data() {
    return {
      ...
      //picker-option(默认是没有的，外部cover进来，这里是示例)
      pickerOptions: pickerOptionsMap['date-point'].month,
      formDataShow: {},
      dataCheckResult: [], //the table-data of auto-form data list show
      dialogVisible: { visible: false }, //dialog-visible
      dataListHead: this.autoForm.getTableHead('autoFormTest_DataList'), //the table-head of auto-form data list show
    };
  },
  computed: {
    colmax() {
    ...
    isShowinControl(list, ID, key) {
      return list.some((item, index) => {
        if (item.hasOwnProperty(ID)) {
          ...
          return ns.base.isXType('boolean', switchVal) && switchVal;
        }
      });
    },
    /**
