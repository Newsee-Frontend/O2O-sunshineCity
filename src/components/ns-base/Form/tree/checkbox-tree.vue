<!-- checkbox-tree 选择框-树状组件 -->
<template>
  <el-tree
    class="cx-simple__tree"
    ref="cxCheckboxTree"
    v-model="childTreeData"
    v-loading="treeSw"
    :data="treeData"
    :props="keyRefer"
    :default-expanded-keys="defaultExpandedKeys"
    :default-checked-keys="defaultCheckedkeys"
    :node-key="keyRefer['value']"
...
import { getInitTreeData, getChildTreeData } from '../../../../service/Form/checkbox-tree';

export default {
  name: 'cx-checkbox-tree',
  data() {
    return {
      childTreeData: null,
  ...
  computed: {
    keyRefer() {
      return this.subconfig.keyRefer;
    },
    outputKeyInList() {
      return this.subconfig.outputKeyInList;
    },
  },
  created() {
  ...
  methods: {
     ...
     */
    getBodyData() {
      if (ns.base.isEmpty(this.treeData)) {
        const url = this.subconfig.initTreeConfig.url; //get init tree data request url
        const query = this.subconfig.initTreeConfig.query; //get init tree data request query

        //request init data
        getInitTreeData(url, query)
          .then(response => {
            //init-leval - 设置默认获取树状结构数据层次
            this.initLeval = parseInt(response.resultData.level);
            //set tree data
            this.treeData = response.resultData[this.keyRefer['firstVos']];
            if (this.keyRefer['isLeaf']) {
              this.treeData.map(o => {
                o[this.keyRefer['isLeaf']] = !o['isHasChild'];
              });
            }
            // set expanded keys array （默认展开）
            this.defaultExpandedKeys = this.setExpandedKeys(
              this.treeData,
              this.initLeval,
              this.expandedIndex
            );
    ...
    loadNode(node, resolve) {
      const ID = node.data[this.keyRefer['value']]; //get tree node id
      if (!ns.base.isEmpty(this.treeData)) {
        //按需获取节点数据 - 所点击节点的数据层级需要在初始展开层级之外
        if (parseInt(node.level) >= this.initLeval) {
          let data = [];
          const url = this.subconfig.childTreeConfig.url; //url for get child node data
          const query = { id: ID }; //request for get child node data
          //get init data （按需获取子节点数据）
          getChildTreeData(url, query)
            .then(response => {
              data = response.resultData[this.keyRefer['firstVos']];
              if (this.keyRefer['isLeaf']) {
                data.map(o => {
                  o[this.keyRefer['isLeaf']] = !o['isHasChild'];
                });
              ...
              resolve(data);
              this.$refs.cxCheckboxTree.setCheckedKeys(
                this.multiple ? this.LastCheck : [this.LastCheck]
              ); //set checked key
    ...
    setExpandedKeys(data, initLeval, expandedIndex) {
      ...
      const firstVos = this.keyRefer.firstVos;
      const ID = this.keyRefer['value'];
      let i = 0;
      if (item) {
        do {
          const ei = expandedIndex[i];
          if (!ns.base.arrContainObj(arr, item[ei][ID])) {
            arr.push(item[ei][ID]);
          }
          if (item[ei].hasOwnProperty(firstVos)) {
            item = item[ei][firstVos];
          }
          i++;
        } while (i < initLeval);
      }
    ...
    handleCheckChange(data, checked, indeterminate) {
      ...
      const model = {};
      this.outputKeyInList.forEach(item => {
        model[item] = data[item];
      });
      console.log('当前点击操作节点的组装对象值');
      console.log(model);
      console.log('当前点击操作节点的组装对象值');
      //multiple situation
      if (this.multiple) {
        if (checked) {
          if (!ns.base.arrContainObj(this.LastCheck, ID)) {
            this.LastCheck.push(ID);
          }
          //model data 数组中不包含 新选中的 对象， 则新加入
          if (!ns.base.arrContainObj(this.childTreeData, model)) {
            this.childTreeData.push(model);
          }
          if (!ns.base.arrContainObj(this.defaultCheckedkeys, ID)) {
  ...
  props: {
    ...
    subconfig: {
      ...
      validator: function(data) {
        if (!data) {
          console.error(
            '[NS-UI warn]: ' + 'attribute configis for checkbox-tree component is not found.'
          );
        }
        if (!data.initTreeConfig || !data.childTreeConfig || !data.keyRefer) {
          console.error(
            '[NS-UI warn]: ' +
              'The initTreeConfig, childTreeConfig and keyRefer field value must exist, please find it.'
          );
        }
        if (
          typeof data.initTreeConfig.url !== 'string' ||
          typeof data.childTreeConfig.url !== 'string'
        ) {
          console.error(
            '[NS-UI warn]: ' +
              "The initTreeConfig's url and childTreeConfig's url field value must be string format, please find it."
          );
        }
        try {
          if (data.outputKeyInList.length === 0) {
            console.warn(
              "The field list can not be empty in output List for checkbox-tree component， Please configure the 'outputKeyInList' field in the configuration file."
            );
          }
        } catch (e) {
          data.outputKeyInList = [data.keyRefer['value'], data.keyRefer['label']];
        }
        return true;
      },
    },
  ...
  watch: {
    defaultCheckedkeys(value) {
      this.$refs.cxCheckboxTree.setCheckedKeys(this.defaultCheckedkeys);
    },
    childTreeData: {
