  ...
  export default {
    ...
    computed: {
      ...
      treeData() {
        ...
        console.log(this.organizeTreeData);
        console.log([this.organizeTreeData])
        return [this.organizeTreeData];
      },
    },
    watch: {
      organizeTreeData: {
    ...
    methods: {
      ...
      treeDataHandle(isFirst) {
        ...
        //树数据--请求
        this.$store.dispatch('getOrganizeTreeData', query).then(res => {
          //树数据--更新数据
          this.$refs.organizeTree.initTree(this.treeData);
          this.childKey && (this.$refs.organizeTree.nodeSelectedByKey([this.childKey]));
      ...
      getTreeData(isFirst) {
        ...
        this.$store.dispatch('getOrganizeTreeData', query).then(data => {
          console.log(99999999999);
          console.log(this.treeData);
          this.$refs.organizeTree.initTree(this.treeData);
          //默认选中第一项
          this.childKey = data.organizationId;
          this.$refs.organizeTree.nodeSelectedByKey([this.childKey]);
          this.searchConditions.organizationId = data.organizationId;
          this.searchConditions.companyId = data.companyId;
          this.searchConditions.departmentId = data.departmentId;
          this.treeloading = false;
          this.$emit('handle-click', null, true);
          this.$emit('tree-item-click', data);
        });
      },
    },
    created() {
