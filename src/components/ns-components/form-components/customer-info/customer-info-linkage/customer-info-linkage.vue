...
<template>
  <div class="customer-info-linkage " v-model="childCustomLinkage">
    <div
      v-for="(firstItem, firstIndex) in linkageInfo"
      :index="firstIndex"
      :key="firstIndex"
      v-if="linkageInfo"
      :class="[
        'customer-info-linkage-item clear',
        { 'ns-required': firstItem.isRequired },
        { 'ns-notRequired': !firstItem.isRequired },
      ]"
    >
      <!--label 部分 一 -->
      <label
        :for="firstItem.modelKey"
        :class="['ns-label fl']"
...
import {
  ...
  choiceRemoteMethod,
  customerInfo,
} from '../../../../ns-base/Form/select/remoteFunc';

export default {
  name: 'ns-customer-info-linkage',
  data() {
    return {
      childCustomLinkage: {},
      options: [],
  ...
  watch: {
    ...
    fatherCustomLinkage() {
      ...
      let index = this.options.findIndex(o => {
        return o.value === this.fatherCustomLinkage.ownerId;
      });
      if (index === -1) {
        this.options.push({
          label: this.fatherCustomLinkage.ownerName,
          value: this.fatherCustomLinkage.ownerId,
        });
      }
      this.linkageInfo[0].modelVal = this.fatherCustomLinkage.ownerId;
    },
  },
};
</script>
