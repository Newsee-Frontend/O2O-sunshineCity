...
<template>
  <el-select
    class="ns-select ns-role-employee ns-select-multiple"
  ...
  >
    ...
    >
      <el-option
        v-for="(item, secondIndex) in group[firstKeyRefer['children']]"
...
export default {
  ...
  methods: {
    ...
    getInitRoleGroupList(query) {
      if (!ns.base.isEmpty(query) && query) {
        getInitRoleGroupList(query)
          .then(response => {
    ...
    synSelection(options) {
      options.map((firstItem, i) => {
        firstItem[this.firstKeyRefer['children']].map((secondItem, j) => {
          secondItem[this.secondKeyRefer['checkSw']] = ns.base.arrContainObj(
            this.childRoleGroup,
            secondItem[this.secondKeyRefer['value']]
          )
            ? true
            : false;
        });
