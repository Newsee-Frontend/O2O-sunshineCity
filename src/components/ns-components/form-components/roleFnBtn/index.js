...
export function createInitModel(vm, options, modelData) {
  ...
  if (!modelData || ns.base.isEmpty(modelData)) {
    console.log(
      '功能菜单按钮的modelData值不存在，格式不正确（非数组），或者为空数组,其值如下：',
...
export function watchModel(vm, val) {
  ...
  // console.info(opts);
  // console.log('childRoleFnBtn 的值如下');
  // console.info(val);
  ...
  SH.loopTemp('first', opts, index, vm.firstStatus, (firstItem, i) => {
    ...
    SH.loopTemp(
      'second',
      firstItem[firstKeyRefer['children']],
      index,
      firstItem,
      (secondItem, j) => {
        //funcinfoVo proptype
...
export class checkAllMethods {
  ...
  checkAllModulesChange(param) {
    if (!param || ns.base.isEmpty(param)) {
      return;
    }
  ...
  checkAllItemsChange(param) {
    ...
    const valList =
      param.vm.childRoleFnBtn[param.firstIndex][modelKeyRefer['infoVo']][param.secondIndex][
        modelKeyRefer['btnItems']
      ]; //model value list
    ...
    btnList.map((typeItem, thirdIndex) => {
      typeItem[thirdKeyRefer['children']].map((btnItem, j) => {
        ...
        //check all => true
        if (param.value) {
          if (!ns.base.arrContainObj(valList, val)) {
